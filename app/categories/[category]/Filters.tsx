            ...
            {showFilters && (
              <div className="col-span-2 space-y-3">
                ...
                {filtersArr.map((item, index) => (
                  <div className="flex cursor-pointer items-center gap-3" key={index} onClick={() => loadFilteredProducts(item)}>
              ...
              {products.products.map((item, index) => (
                <div key={index}>
                  <Link href={`/categories/${category}/${item.node.id}`} className="block h-full max-h-[358px] w-full rounded-md border">
                    {item.node.images.edges[0].node.url ? <Image width={500} height={500} src={item.node.images.edges[0].node.url} className="size-full rounded-md object-cover" alt={item.node.images.edges[0].node.altText} /> : <div className="flex h-full min-h-[175px] items-center justify-center rounded-md border border-white/15 bg-black sm:min-h-[275px] lg:min-h-[400px]">No Image Found</div>}
                  </Link>
                </div>
              ))}
            </div>
